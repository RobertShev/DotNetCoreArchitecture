...
Some features and practices were made to facilitate learning and make the code as small as possible.

For a real project, consider the following suggestions:

* Do not use technologies and features that are not necessary.
* Use DDD concepts in the right way, such as ubiquitous language, to express the domain.
...
* Apply microservices if the domain contexts are defined, if each microservice has a database, if apply DevOps, etc.
* Apply SignalR for real-time communication, avoid calls in every x time.
